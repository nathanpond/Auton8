...
- **Agent skills are enforced, not trusted.** `tests/AutoNate.Web.Tests/Security/AgentSkillAuthorizationTests.cs` classifies every `IAgentSkill` as `Authorizer` (calls an authorizer itself), `GatedStore` (reads via a store that applies `IAuthorizer` internally, e.g. `EfCoreRecordStore`'s `BuildRecordSqlFilterAsync`/`FilterQueryAsync`), `ActorScopedStore` (`*ForActorAsync`/`*ForUserAsync` only) or `NoGatedData`. **A new skill fails that test until it is classified**, and one classified `Authorizer` fails if its authorizer call is removed. Mirror the gate the REST route over the same store uses — kind-level for lists, per-instance for a single get — and word a denial identically to a miss so a tool can't enumerate ids (#19, #20). Note the notes skills authorize through `IContentAuthorizer`, not `IAuthorizer`.

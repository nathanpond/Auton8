...
- `.n8/config.yml`: stack `dotnet`, default branch `master`, wiki opted out (docs live in `docs/`), issue areas `api|spa|plugins|services|flowable|infra|ci|docs|tests`. `.github/` has Dependabot (weekly grouped minor/patch for nuget, npm ×4, maven, actions) issue templates, and `workflows/ci.yml` (added after this map was generated).

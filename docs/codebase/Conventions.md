...
- Scripts (`src/AutoNate.Spa/package.json:6-12`): `npm run dev`, `npm run build` (= `tsc -b && vite build`), `npm run type-check`, `npm run lint` (= `eslint src --max-warnings=411`).
- **Warning ratchet:** `--max-warnings=411` is the current count; it may only go **down**. If your change adds warnings, fix them; if it removes some, lower the number in the same commit (commit `8674f2fb` set it from 999 → 411).

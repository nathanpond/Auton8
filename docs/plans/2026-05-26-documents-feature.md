...
2. **Phase 11 — read-only preview.** Highest user-facing value, and Phase 10 made it cheap: resolved values now live in the doc tree, so preview is just a read-only mount of the live doc — **the `resolveBindingsInPmDoc` helper (§9c/§9d) is no longer needed**. Add `/documents/preview/:documentId` (a `usePageKey` placeholder already exists) + a "Preview" button beside "Download .docx".

3. **Phase 9 — polish.** Independent, medium value. Two pieces remain (refresh-all bindings already shipped): the **version-history sidebar** (docx-editor exposes hooks) and the **folder/document override-permission editor**.

4. **Phase 10c — text-binding control polish.** Confirm scope first — the Phase 10b commit flagged it as pending but the rollout table marks Phase 10 complete. Thematically adjacent to the binding work just finished, likely small; sequence it once it's clear what it covers.

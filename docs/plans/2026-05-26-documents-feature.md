...
- **Deferred to v2**: unifying folders with notes' cabinet/notebook tree; **importing a `.docx` into an existing document with diff/merge UI** (paragraph-level diff, accept/reject per chunk, applied as a Yjs transaction).
...
(DOCX-into-existing-document merge/diff is intentionally not in the rollout — v2.)
...
5. ~~**Phase 8c — NL→AQL "suggest a binding" dialog.**~~ ✅ **Done (2026-05-28).** A focused, one-shot NL→AQL counterpart to the agent's `AqlAssistSkill`. New `AqlSuggestionService` resolves the agent's configured LLM provider (Anthropic→OpenAI), prompts it with the AQL grammar + the live entity catalog (from `IQueryEntityRegistry`) for a JSON `{query, explanation}`, then **validates server-side** with `AqlParser`/`AqlValidator` and **retries once** feeding the errors back. New `POST /api/aql/suggest` (any authed user — drafts query TEXT only, never executes). Frontend: `api/aqlSuggest.ts` + `useSuggestAql`, and a "Describe it in plain English" field + "Suggest query" button in the aql-table binding form (insert + edit) that fills the AQL query and shows the explanation + a `Validated ✓` / `may need tweaks` badge. Verified end-to-end: "all cars sorted by name" → `FROM Records WHERE RecordType = "Car" ORDER BY Name ASC` (valid); "last two weeks of workflow runs" → `... StartDate >= -2w ...` (correct relative-date syntax, not quoted); "5 most recently created cars" filled the field with a valid query + Validated badge in the UI; an invalid draft surfaces the validator's error. (Tuned the prompt's grouped-aggregate example, which had ORDER-BY-after-COLUMNS — the wrong clause order — so grouped requests come back valid.)

6. **Phase 8d — template + prompt → generate-a-document.** Largest remaining AI piece; do last.

**Still deferred to v2** (not scheduled): unifying folders with the notes cabinet/notebook tree, and importing a `.docx` into an *existing* document with diff/merge UI.

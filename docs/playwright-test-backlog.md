...
| E2E-029 | DOCX import lifecycle | Upload a `.docx`, verify the import editor opens, and confirm imported content is committed. | High | `DocumentEditorTests.cs` | DONE | Unblocked by archived-173: the import finalize hung off a docx-editor `onChange` that never fires during the OOXML parse, so `?import=1` never cleared. Finalize now watches the parsed document settle; `ProjectDocuments_ImportDocx_CommitsImportedContent` is un-skipped and asserts the committed body_jsonb. |
...
| E2E-044 | Limited user admin-route gating | Sign in as a limited user and verify protected admin pages are inaccessible. | High | `PermissionGatingTests.cs` | DONE | Unblocked by adding `shell/PermissionRoute.tsx` and gating `admin/config` on `siteconfig:view` — the same pair the config endpoints declare server-side. Covered by `LimitedUser_DeepLinkingIntoAdminConfig_SeesNoAccessNotTheShell` and its positive twin `AfterGrantingSiteConfigView_LimitedUser_ReachesAdminConfig`. |
| E2E-045 | Limited user record-action gating | Grant record view without delete, verify delete is hidden, then grant delete and verify it appears. | High | `PermissionGatingTests.cs` | DONE | Unblocked by gating RecordDetail's delete on an instance-level `record:delete` check via `usePermissionChecks`. Covered by `LimitedUser_RecordDetail_DeleteActionAbsentUntilGranted`, which grants delete mid-test so the absence is proven to be the permission check rather than a mis-locator. |
...
| E2E-061 | Typed record fields and filters | Configure representative field types, create matching records, apply typed filters, and verify filtered results and selected columns. | High | `RecordsAdvancedTests.cs` | PARTIAL | Unblocked: `ApiSeeder.AddRecordTypeFieldAsync` builds typed schemas, and the filter row's three controls are now named (`Filter N field/operator/value`) so a spec can target them semantically. `TypedFields_FilterTheRecordListToMatchingRowsOnly` covers text/number/option schema plus an option filter narrowing the list; verified by inverting the filter value and watching the surviving row swap. Column-picker selection is still uncovered. |
...
| E2E-066 | Configuration section persistence | Persist appearance, status-appearance, feature, chatbot, event, and system-health settings and verify reload behavior. | Medium | `AdminOperationsTests.cs` | DONE | Not a product defect: the spec reloaded on the line after the Save click and the navigation aborted the in-flight PATCH. It now waits for the page's own "Appearance settings saved." alert. See archived-172. |
...
| E2E-069 | OIDC interop against real Keycloak | Sign in through the seeded Keycloak realm over OIDC and assert the resulting session, not just the landing URL. | High | `IdentityProviderInteropTests.cs` | DONE | Traited `RequiresService=Keycloak`, so it belongs to the future **full-keycloak** tier (#479) — neither slim nor full-local selects it. #90's stub proves the implementation matches one reading of the spec; this proves it works against software somebody else wrote. |
| E2E-070 | SAML interop against real Keycloak | Same journey over SAML — the half where interop actually breaks (canonicalization, NameID format, attribute encoding). | High | `IdentityProviderInteropTests.cs` | DONE | The fixture creates the SAML client per run because the SP entity ID *is* the client id and is derived from the app's random port. Keycloak stores a mid-string port wildcard and then rejects it at authorization time; only trailing wildcards are honoured. |
| E2E-071 | Claim mapping through both protocols | A Keycloak user in a mapped group holds the Auton8 group; removing them at the IdP removes it at the next sign-in. | High | `IdentityProviderInteropTests.cs` | DONE | Both directions, both protocols. The revocation half is the one a first implementation silently never does, and it was previously proven only against our own reconciler. |
| E2E-072 | Account matching against real tokens | The same Keycloak subject with a changed email resolves to one Auton8 account, not two. | High | `IdentityProviderInteropTests.cs` | DONE | Matching is on the subject; keying on email would strand everything the first account held. |

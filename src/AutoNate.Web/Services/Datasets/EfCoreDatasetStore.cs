...
#pragma warning restore CA1304, CA1311
        ...
        CreateDatasetInput input, Guid actorId, CancellationToken cancellationToken = default)
    {
        ...
        var entity = new Dataset
        {
            ...
            FileScopeKind = string.IsNullOrWhiteSpace(input.FileScopeKind) ? null : input.FileScopeKind.Trim(),
            FileScopePath = string.IsNullOrWhiteSpace(input.FileScopePath) ? null : input.FileScopePath.Trim(),
            ParserKind = string.IsNullOrWhiteSpace(input.ParserKind) ? null : input.ParserKind.Trim(),
            ParserOptionsJson = string.IsNullOrWhiteSpace(input.ParserOptionsJson) ? null : input.ParserOptionsJson,
            OwnerUserId = actorId,
            CreatedAtUtc = now,
            UpdatedAtUtc = now,
            CreatedBy = actorId,

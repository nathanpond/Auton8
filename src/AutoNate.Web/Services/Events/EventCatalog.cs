...
public static class EventCatalog
{
    ...
    [
        ...
        new("resource.role", "'owner' | 'contributor' | 'commenter' | 'viewer'", "Project role for membership events."),
    ...
    [
        ...
        new(
            ...
            [
                ...
                new EventCatalogEntry(ContentEventTopic.TopicName, ContentEventTypes.FolderCreated,
                    "A folder was created under a project (root) or another folder.",
                    "Fires from POST /api/content/folders.",
                    ["resource: { id, projectId, parentFolderId, name }. details: null."]),
                new EventCatalogEntry(ContentEventTopic.TopicName, ContentEventTypes.FolderUpdated,
                    "A folder was renamed / re-described / re-iconed / re-ordered / archived.",
                    "Fires from PATCH /api/content/folders/{id}.",
                    ["resource: { id, name }. details: { fields }."]),
                new EventCatalogEntry(ContentEventTopic.TopicName, ContentEventTypes.FolderMoved,
                    "A folder was moved to a different parent (project root or another folder). Closure rows are rebuilt.",
                    "Fires from PATCH /api/content/folders/{id} when project_id or parent_folder_id changes.",
                    ["resource: { id }. details: { previousProjectId, newProjectId, previousParentFolderId, newParentFolderId }."]),
                new EventCatalogEntry(ContentEventTopic.TopicName, ContentEventTypes.FolderDeleted,
                    "A folder was deleted. Cascade-deletes descendant folders (and, once documents ship, contained documents).",
                    "Fires from DELETE /api/content/folders/{id}. Refused under deletions_locked.",
                    ["resource: { id }. details: null."]),
                new EventCatalogEntry(ContentEventTopic.TopicName, ContentEventTypes.DocumentCreated,
                    "A document or template was created. `details.kind` discriminates 'document' vs 'template'.",
                    "Fires from POST /api/content/documents. The initial body version (v1) is recorded in the same tx.",
                    ["resource: { id, projectId, folderId, title }. details: { kind, templateId }."]),
                new EventCatalogEntry(ContentEventTopic.TopicName, ContentEventTypes.DocumentUpdated,
                    "A document was updated. Title/body changes trigger a version snapshot (rolled up into the same editing-session row when applicable).",
                    "Fires from PATCH /api/content/documents/{id}.",
                    ["resource: { id, title }. details: { fields, newVersionNumber } — newVersionNumber present only when a fresh row was written."]),
                new EventCatalogEntry(ContentEventTopic.TopicName, ContentEventTypes.DocumentMoved,
                    "A document was moved (different folder and/or different project). Closure rows are rebuilt for the subtree.",
                    "Fires from PATCH /api/content/documents/{id} when project_id or folder_id changes.",
                    ["resource: { id }. details: { previousProjectId, newProjectId, previousFolderId, newFolderId }."]),
                new EventCatalogEntry(ContentEventTopic.TopicName, ContentEventTypes.DocumentDeleted,
                    "A document was deleted. Cascade-deletes its version rows.",
                    "Fires from DELETE /api/content/documents/{id}. Refused under deletions_locked.",
                    ["resource: { id }. details: null."]),
                new EventCatalogEntry(ContentEventTopic.TopicName, ContentEventTypes.DocumentVersionCreated,
                    "A document version row was written (autosave session-start, manual save, or restore snapshot). The version row carries the *prior* state.",
                    "Fires from PATCH /api/content/documents/{id} after a title/body change.",
                    ["resource: { documentId, versionNumber, kind }. details: null."]),
                new EventCatalogEntry(ContentEventTopic.TopicName, ContentEventTypes.DocumentVersionRestored,
                    "A document was reset to a prior version. The current state is first captured as a kind='restore' version.",
                    "Fires from POST /api/content/documents/{id}/versions/{n}/restore.",
                    ["resource: { documentId, restoredFromVersion, snapshotVersionNumber }. details: null."]),
                new EventCatalogEntry(ContentEventTopic.TopicName, ContentEventTypes.DocumentVersionDeleted,
                    "A non-current document version was pruned.",
                    "Fires from DELETE /api/content/documents/{id}/versions/{n}. Refused under deletions_locked, on the current version, or on the only existing version.",
                    ["resource: { documentId, versionNumber }. details: null."]),

...
function LiveDiagram({
  ...
  // Viewer-role short-circuits the drawio iframe entirely:
  //   - has a snapshot → render the SVG inline (cheap, no iframe boot)
  //   - no snapshot    → blank/un-edited diagram; show "no content"
  // Editor-role users always go through the iframe so they can draw.
  if (viewer) {
    return (
      <DiagramShell
        title={title}
  ...
  return (
    <div
      style={{
        ...
        // Generous top padding so the diagram doesn't hug the title bar.
...
// Shown to viewer-role users when the diagram has no snapshot yet (the
// editor never autosaved, so there's nothing to render).
function ViewerEmptyPane() {
  return (
    <div
      style={{
        flex: 1,
    ...
    >
      <div style={{ color: notesTheme.muted, fontSize: 13 }}>
        This diagram has no content yet.
      </div>
    </div>
  );
}

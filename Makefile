...
keycloak-up: infra-prepare keycloak-check
	...
	#
	# The marker has to be the line IMMEDIATELY before the finding — Semgrep does
	# not look further back, so the explanation goes above it rather than between.
	# nosemgrep: generic.secrets.security.detected-username-and-password-in-uri

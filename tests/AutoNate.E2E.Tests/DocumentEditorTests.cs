...
public sealed class DocumentEditorTests : E2ETestBase
{
    ...
    public async Task DocumentEditor_ContentPersistsAcrossReload()
    {
        ...
        await Assertions.Expect(editor).ToContainTextAsync(bodyText, new() { Timeout = 30_000 });
    }

    [Fact(Skip = "Blocked: DOCX import upload navigates to ?import=1, but the editor wrapper never finalizes parsed content; editor-core-generated DOCX fixtures reload with an empty body.")]
